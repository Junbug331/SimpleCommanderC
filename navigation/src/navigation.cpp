...
#define NAVIGATE_THROUGH_POSES 7
...
Navigator::Navigator(bool debug, bool verbose) : rclcpp::Node("navigator")
{
    ...
    localization_pose_subscriber = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>("amcl_pose", 10, std::bind(&Navigator::amcl_pose_callback, this, std::placeholders::_1));
...
bool Navigator::NavigateThroughPoses(const std::vector<geometry_msgs::msg::PoseStamped> &poses)
{
    using namespace std::placeholders;
    if (debug)
        RCLCPP_INFO(get_logger(), "Waiting for NavigateThroughPoses action server");
    navigate_through_poses_client->wait_for_action_server();
    auto goal_msg  = nav2_msgs::action::NavigateThroughPoses::Goal();
    goal_msg.poses = poses;

    current_executing      = NAVIGATE_THROUGH_POSES;
    auto send_goal_options = rclcpp_action::Client<nav2_msgs::action::NavigateThroughPoses>::SendGoalOptions();

    send_goal_options.feedback_callback =
            std::bind(&Navigator::generic_feedback_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::NavigateThroughPoses>::SharedPtr,
                                                            const std::shared_ptr<const nav2_msgs::action::NavigateThroughPoses::Feedback>>,
                      this, _1, _2);

    send_goal_options.result_callback =
            std::bind(&Navigator::generic_result_callback<rclcpp_action::ClientGoalHandle<nav2_msgs::action::NavigateThroughPoses>::WrappedResult>, this, _1);

    auto send_goal_future = navigate_through_poses_client->async_send_goal(goal_msg, send_goal_options);
    rclcpp::spin_until_future_complete(this->get_node_base_interface(), send_goal_future);
    auto goal_handle = send_goal_future.get();
    if (goal_handle != NULL)
    {
    ...
    future_navigate_through_poses = navigate_through_poses_client->async_get_result(goal_handle);
